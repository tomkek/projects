...
#define MAX_POSITIVE_ACCELERATION 1900 // Pulse width for max negative acceleration (to be experimentally validated).
#define MAX_NEGATIVE_ACCELERATION 800 // Pulse width for max negative acceleration (to be experimentally validated).
...
void setMotorData(int steering, int speed) {
   ...
   steeringServo = CENTER_STEERING + (steering * 11.75);
    ...
    // Correct the entered value for the motor if they are not not in the correct range.
    ...
    desiredSpeed = NO_ACCELERATION;
    
    // Step-wise increasing or decreasing the PWM --> this needs to be validated on the car. 
/*
...
void initializeSteeringAcceleration(void) {
    ...
    palSetPadMode(GPIOC, 9, PAL_MODE_ALTERNATE(STM32F4GPIO_AF_TIM3));
